        ...
        <div class="col-md-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>Edit Union Council</h5>
                </div>
                <div class="ibox-content pb-1">
                    <form class="m-t" role="form" method="post"
                        action="{{ route('union-councils.update', $unionCouncil->id) }}" id="form_exam_center">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input name="name" id="name" type="text"
                                        ...
                                        placeholder="Please Enter Union Council Name"
                                        value="{{ $unionCouncil->name }}">
                                    @error('name')
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input name="abbreviation" id="abbreviation" type="abbreviation"
                                        ...
                                        value="{{ $unionCouncil->abbreviation }}">
                                    @error('abbreviation')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                    ...
                                    <select class="form-control  @error('taluka_id') is-invalid @enderror"
                                        ...
                                        @foreach ($talukas as $single)
                                            <option value="{{ $single->id }}"
                                                {{ $single->id == $unionCouncil->taluka_id ? 'selected' : '' }}>
                                                {{ $single->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('taluka_id')
