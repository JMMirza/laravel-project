            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content pb-1">
                    <form class="m-t" role="form" method="post"
                        action="{{ route('schools.update', $school->id) }}" id="form_exam_center">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input name="name" id="name" type="text"
                                        ...
                                        placeholder="Please Enter School Name" value="{{ $school->name }}">
                                    @error('name')
                                    ...
                                    <select class="form-control  @error('union_council_id') is-invalid @enderror"
                                        ...
                                        @foreach ($union_councils as $single)
                                            <option value="{{ $single->id }}"
                                                {{ $single->id == $school->union_council_id ? 'selected' : '' }}>
                                                {{ $single->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control  @error('address') is-invalid @enderror" placeholder="Please Enter School Address"
                                        name="address" id="address">{{ $school->address }}</textarea>
                                    @error('address')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
