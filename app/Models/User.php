...

{
    ...
    public function getDomicileUrlAttribute()
    {
        $image = asset('uploads/picture-not-available.jpg');

        if (!empty($this->domicile) && file_exists('uploads/candidates_images/' . $this->domicile)) {
            $image = asset('uploads/candidates_images/' . $this->domicile);
        }
    ...
    public function getIsJobApplicationExistAttribute()
    {
        $result = JobApplication::where(['user_id' => $this->id])->first();
        $checks = 'false';
        if ($result) {
